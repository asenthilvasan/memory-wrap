...
absl.flags.DEFINE_string("path_aes", None,
                         "Path of the trained autoencoder model")
absl.flags.DEFINE_string("dir_dataset", '../datasets/',
                         "Path of the trained autoencoder model")
absl.flags.DEFINE_enum("algo", None, ['memory', 'proto'],
                       "Algorithm for computing counterfactuals")
absl.flags.mark_flag_as_required("path_models")
absl.flags.mark_flag_as_required("algo")


def get_counterfactuals_indices(sorted_indices: torch.Tensor,
                                predictions: torch.Tensor,
                                targets: torch.Tensor) -> torch.Tensor:
    """
    Get the indices of the counterfactuals in memory.

    Given a sorted indices tensor, the predictions and the targets, this
    function returns the indices of the counterfactuals in memory. Namely,
    it extracts the indices from the sorted indices for which the prediction
    ...
    in_memory = (sorted_pred != targets.unsqueeze(dim=1).repeat(
...
def get_prototypes_from_memorynet(model: torch.nn.Module,
                                  enc_model: torch.nn.Module,
                                  memory: torch.Tensor,
                                  auxiliary_memory: torch.Tensor) -> None:
    """
...
def get_closest_prototypes(model, input_data, class_prototypes,
                           class_encodings, k=None, k_type='mean',
                           target_class=None):
    """
    ...
    for _ in range(encoded_input.shape[0]):
        for class_proto, prototype in enumerate(class_dict):
            if class_proto not in target_class:
                continue
            if k is None:
                dist_proto[class_proto] = torch.linalg.norm(
                    encoded_input - prototype)
            elif k is not None:
                dist_k = torch.linalg.norm(
                    encoded_input.reshape(encoded_input.shape[0], -1) -
...
def run_proto_cf(path_models: str, path_aes: str, path_dataset: str):
    ...
    Args:
        ...
        path_aes (str): Path of the trained autoencoder models for the
            path_models.
        path_dataset (str): Path where the dataset is stored.
    """
    # load name of models in the path_models dir
    list_models = [name_file for name_file in os.listdir(path_models)
    ...
    for index_model, name_model in enumerate(list_models):
        ...
        ae_fulldataset.load_state_dict(
            torch.load(f"{path_aes}/{seed}/full.pt"))
        ...
        if FLAGS.algo == 'proto':
            ...
            fulldataset_loader, _, _, _ = load_dataset(
                        dataset_dir,
                        ...
                        seed=seed)
            class_prototypes = get_prototypes_from_memorynet(
                        model=model.cpu(),
        ...
        for batch_idx, data in enumerate(test_loader):
            ...
            ################ Input  ############################
            ####################################################
            # keep copy on the cpu to compute closest prototype
            img_cpu, _ = data
            memory_input, _ = next(iter(mem_loader))
            ...
            # get output
            with torch.no_grad():
                logits, weights = model(img_gpu, memory_input,
                                        return_weights=True)

                # get predicted class and its one-hot encoding
                predicted_class = torch.argmax(logits, axis=1)
                onehot_prediction = nn.functional.one_hot(predicted_class,
                                                          num_classes=classes)
            ...
            sorted_weights, sorted_weights_indices = torch.sort(
                weights, descending=True)
            counterfactuals_candidates = get_counterfactuals_indices(
                sorted_weights_indices, memory_predictions, predicted_class)
            ...
            count_counterfactuals = torch.count_nonzero(
                counterfactuals_candidates*sorted_weights, 1)
            ...
            if samples_with_cf.size()[0] > 1:
                ...
                prediction_top_counterfactual = memory_predictions[
                    index_top_counterfactual]
            ...
            if counterfactual is not None:
                orig_pred_class = torch.flatten(
                    torch.argmax(onehot_prediction, axis=1)
                    [count_counterfactuals.nonzero()])

                counterfactual = counterfactual.cuda()
                counterfactual = counterfactual.squeeze(1)
                cf_pred_class = torch.argmax(
                    model(counterfactual, aux_memory), axis=1)
                ...
                for or_class, cf_class in zip(orig_pred_class, cf_pred_class):
                    ...
                    im1_score = counterfactuals_utils.compute_im1_score(
                        counterfactual, ae_cf, ae_orig)
                    iim1_score = counterfactuals_utils.compute_iim1_score(
                        counterfactual, ae_cf, ae_orig)
                    im2_score = counterfactuals_utils.compute_im2_score(
                        counterfactual, ae_orig, ae_fulldataset)
